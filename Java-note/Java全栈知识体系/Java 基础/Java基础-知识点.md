...
|         | 装箱方法                             | 拆箱方法                     |
| ------- | ------------------------------------ | ---------------------------- |
| 总结    | [包装类型].valueOf(基础类型)         | [包装对象].[基础类型]Value() |
| Integer | public static Integer valueOf(int i) | int intValue()               |
...
```java
Integer x = 2;     // 装箱 Integer.valueOf() 等价于 Integer x = Integer.valueOf(100);
int y = x;         // 拆箱 x.intValue() 等价于 int y = x.intValue();
```



**可以通过字节码，和debug验证： 装箱拆箱的发生**

![image-20230527172051411](img/img_Java%E5%9F%BA%E7%A1%80-%E7%9F%A5%E8%AF%86%E7%82%B9/image-20230527172051411.png)
...
**包装类型和基础类型进行比较： 拆箱；如果包装类型为null，则会报空指针异常**。**基本类型只能通过`==`比较**

```java
Integer x = 2;     // 装箱 Integer.valueOf() 等价于 Integer x = Integer.valueOf(100);
int y = x;         // 拆箱 x.intValue() 等价于 int y = x.intValue();
System.out.println(x == 2); //拆箱 System.out.println(x.intValue() == 2)
Integer z = null;
System.out.println(z == 1); // 拆箱，空指针异常
```

```java
Exception in thread "main" java.lang.NullPointerException: Cannot invoke "java.lang.Integer.intValue()" because "z" is null
	at com.lmz.basic.basic_type.IntegerTest.main(IntegerTest.java:22)
```
...
![img](../../../../../../images/pics/f76067a5-7d5f-4135-9549-8199c77d8f1c.jpg)
