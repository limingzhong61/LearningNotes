...
在linux中，不管是不同文件还是socket，都用一个文件描述符FD表示

#### 4）TCP_NODELAY

* 属于 SocketChannal 参数

nagle是受到对方ack前攒，接收后直接发送吧

nagle默认是开启（false）的，最好关闭掉

![image-20230616223026538](img/img_Netty04-%E4%BC%98%E5%8C%96%E4%B8%8E%E6%BA%90%E7%A0%81/image-20230616223026538.png)
...
![image-20230616224207484](img/img_Netty04-%E4%BC%98%E5%8C%96%E4%B8%8E%E6%BA%90%E7%A0%81/image-20230616224207484.png)

池化的目的就是避免内存的多次分配申请，一次性申请完复用

#### 7）RCVBUF_ALLOCATOR

* 属于 SocketChannal 参数
* **控制 netty 接收缓冲区大小**
* 负责入站数据的分配，决定入站缓冲区的大小（并**可动态调整**），**统一采用 direct 直接内存**，具体池化还是非池化由 allocator 决定

![image-20230616224854322](img/img_Netty04-%E4%BC%98%E5%8C%96%E4%B8%8E%E6%BA%90%E7%A0%81/image-20230616224854322.png)

很明显了 alloc那个是用来写的时候或者接收本地磁盘io可以用的buf 而rcv是netty网络传输读数据用的buf
...
总结：为了使调用方能够简单使用远程调用，避免直接面对netty发送消息的形式进行远程调用，所以**封装了代理类，只需要给代理类传递要调用的接口类型，就可以通过动态代理封装消息进行远程调用**

> 在客户端这边，HelloService是怎么获取到的呢？
>
> 接口提供方把提供的XXXService接口打成一个jar提供给调用方调用
>
> 所以企业里一般都是把api接口单独作为一个二方包，其他有人要调你的方法时，只需要引你的二方包就好了





> 这里发消息的2、3两步需要调换下顺序，否则可能出现先接收消息，然后再put promise，导致丢失结果
...
![image-20230618165333337](img/img_Netty04-%E4%BC%98%E5%8C%96%E4%B8%8E%E6%BA%90%E7%A0%81/image-20230618165333337.png)
...
> 比这看: selector是epoll, channel是一个fd, 注册就是epoll_ctl
...
![image-20230618173114555](img/img_Netty04-%E4%BC%98%E5%8C%96%E4%B8%8E%E6%BA%90%E7%A0%81/image-20230618173114555.png)
...
![image-20230618195901029](img/img_Netty04-%E4%BC%98%E5%8C%96%E4%B8%8E%E6%BA%90%E7%A0%81/image-20230618195901029.png)
...
![image-20230618205918402](img/img_Netty04-%E4%BC%98%E5%8C%96%E4%B8%8E%E6%BA%90%E7%A0%81/image-20230618205918402.png)

![image-20230618212223639](img/img_Netty04-%E4%BC%98%E5%8C%96%E4%B8%8E%E6%BA%90%E7%A0%81/image-20230618212223639.png)

![image-20230618212523769](img/img_Netty04-%E4%BC%98%E5%8C%96%E4%B8%8E%E6%BA%90%E7%A0%81/image-20230618212523769.png)
...
> 刚才查了一下，for(;;)编译后只有一行whiletrue编译后四行，for写法占用寄存器少
...
![image-20230618212723643](img/img_Netty04-%E4%BC%98%E5%8C%96%E4%B8%8E%E6%BA%90%E7%A0%81/image-20230618212723643.png)
