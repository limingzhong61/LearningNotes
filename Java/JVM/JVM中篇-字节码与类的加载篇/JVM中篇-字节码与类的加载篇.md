...
![image-20220425202631237](JVM中篇-字节码与类的加载篇/image-20220425202631237.png)

![image-20220425202724670](JVM中篇-字节码与类的加载篇/image-20220425202724670.png)

### Java的前端编译器

![image-20220425202842518](JVM中篇-字节码与类的加载篇/image-20220425202842518.png)

![image-20220425202847475](JVM中篇-字节码与类的加载篇/image-20220425202847475.png)

### 透过字节码指令看代码细节

![image-20220425202859925](JVM中篇-字节码与类的加载篇/image-20220425202859925.png)
...
![image-20220425203222202](JVM中篇-字节码与类的加载篇/image-20220425203222202.png)

![image-20220425203226881](JVM中篇-字节码与类的加载篇/image-20220425203455062.png)

![image-20220425203231864](JVM中篇-字节码与类的加载篇/image-20220425203231864.png)

## 03Class 文件结构

https://docs.oracle.com/javase/specs/jvms/se8/html/jvms-4.html

![image-20220426141513201](JVM中篇-字节码与类的加载篇/image-20220426141513201.png)
...
![image-20220426141522162](JVM中篇-字节码与类的加载篇/image-20220426141522162.png)

### class文件结构概述

![image-20220426141533251](JVM中篇-字节码与类的加载篇/image-20220426141533251.png)![image-20220426141538368](JVM中篇-字节码与类的加载篇/image-20220426141538368.png)![image-20220426141546209](JVM中篇-字节码与类的加载篇/image-20220426141546209.png)

![image-20220426141603094](JVM中篇-字节码与类的加载篇/image-20220426141603094.png)
...
00000060: 65 01 00 04 74 68 69 73 01 00 0F 4C 63 68 61 70    e...this...Lchap
00000070: 74 65 72 31 2F 44 65 6D 6F 3B 01 00 03 61 64 64    ter1/Demo;...add
00000080: 01 00 03 28 29 49 01 00 0A 53 6F 75 72 63 65 46    ...()I...SourceF
00000090: 69 6C 65 01 00 09 44 65 6D 6F 2E 6A 61 76 61 0C    ile...Demo.java.
000000a0: 00 07 00 08 0C 00 05 00 06 01 00 0D 63 68 61 70    ............chap
000000b0: 74 65 72 31 2F 44 65 6D 6F 01 00 10 6A 61 76 61    ter1/Demo...java
000000c0: 2F 6C 61 6E 67 2F 4F 62 6A 65 63 74 00 21 00 03    /lang/Object.!..
000000d0: 00 04 00 00 00 01 00 02 00 05 00 06 00 00 00 02    ................
000000e0: 00 01 00 07 00 08 00 01 00 09 00 00 00 38 00 02    .............8..
000000f0: 00 01 00 00 00 0A 2A B7 00 01 2A 04 B5 00 02 B1    ......*7..*.5..1
00000100: 00 00 00 02 00 0A 00 00 00 0A 00 02 00 00 00 03    ................
00000110: 00 04 00 04 00 0B 00 00 00 0C 00 01 00 00 00 0A    ................
00000120: 00 0C 00 0D 00 00 00 01 00 0E 00 0F 00 01 00 09    ................
00000130: 00 00 00 3D 00 03 00 01 00 00 00 0F 2A 2A B4 00    ...=........**4.
00000140: 02 05 60 B5 00 02 2A B4 00 02 AC 00 00 00 02 00    ..`5..*4..,.....
00000150: 0A 00 00 00 0A 00 02 00 00 00 07 00 0A 00 08 00    ................
00000160: 0B 00 00 00 0C 00 01 00 00 00 0F 00 0C 00 0D 00    ................
00000170: 00 00 01 00 10 00 00 00 02 00 11                   ...........

```

![image-20220426144611159](JVM中篇-字节码与类的加载篇/image-20220426144611159.png)



### 01-魔数：Class文件的标志

![image-20220426145253525](JVM中篇-字节码与类的加载篇/image-20220426145253525.png)

### 02-Class文件版本号

![image-20220426145819140](JVM中篇-字节码与类的加载篇/image-20220426145819140.png)

![image-20220426145829534](JVM中篇-字节码与类的加载篇/image-20220426145829534.png)

### 03-常量池：存放所有常量

![image-20220426151854926](JVM中篇-字节码与类的加载篇/image-20220426151854926.png)

![image-20220426151908521](JVM中篇-字节码与类的加载篇/image-20220426151908521.png)

> jdk8以后**字符串常量池放在了堆空间，其他常量还是在元空间**

#### 1-常量池计数器

![image-20220426152438686](JVM中篇-字节码与类的加载篇/image-20220426152438686.png)

#### 2-常量池表

![image-20220426152707766](JVM中篇-字节码与类的加载篇/image-20220426152707766.png)

![image-20220426152720537](JVM中篇-字节码与类的加载篇/image-20220426152720537.png)

##### 2.1 字面量和符号引用

![image-20220426153003763](JVM中篇-字节码与类的加载篇/image-20220426153003763.png)

![image-20220426153009160](JVM中篇-字节码与类的加载篇/image-20220426153313956.png)

![image-20220426153020359](JVM中篇-字节码与类的加载篇/image-20220426153020359.png)

##### 2.2 常量类型和结构

![image-20220426154450771](JVM中篇-字节码与类的加载篇/image-20220426154450771.png)

![image-20220426154509865](JVM中篇-字节码与类的加载篇/image-20220426154509865.png)

![image-20220426154514749](JVM中篇-字节码与类的加载篇/image-20220426154514749.png)



![image-20220426154519721](JVM中篇-字节码与类的加载篇/image-20220426154519721.png)

![image-20220426154524835](JVM中篇-字节码与类的加载篇/image-20220426154524835.png)

![image-20220426154530642](JVM中篇-字节码与类的加载篇/image-20220426154530642.png)

![image-20220426154535378](JVM中篇-字节码与类的加载篇/image-20220426154535378.png)

### 04-访问标识

![image-20220427141436105](JVM中篇-字节码与类的加载篇/image-20220427141436105.png)

![image-20220427141443223](JVM中篇-字节码与类的加载篇/image-20220427141443223.png)

### 05-类索引、父类索引、接口索引集合

![image-20220427143105499](JVM中篇-字节码与类的加载篇/image-20220427143105499.png)![image-20220427143112778](JVM中篇-字节码与类的加载篇/image-20220427143112778.png)

### 6字段表集合

**fields**

用于描述接口或类中声明的变量。字段（field）包括<mark>类级变量以及实例级变量</mark>，但是不包括方法内部、代码块内部声明的局部变量(local variables)。
...
![image-20220427144507532](JVM中篇-字节码与类的加载篇/image-20220427144507532.png)
...
![image-20220427145041827](JVM中篇-字节码与类的加载篇/image-20220427145041827.png)
...
![image-20210421235232911](JVM中篇-字节码与类的加载篇/412a7e52bfb1ee0aa8229db1402ae58a.png)
...
![image-20220427153643068](JVM中篇-字节码与类的加载篇/image-20220427153643068.png)

# 04-使用javap指令解析Class文件

## 1-解析字节码的作用

![image-20220427154441218](JVM中篇-字节码与类的加载篇/image-20220427154441218.png)

## 2-javac -g操作

![image-20220427154451847](JVM中篇-字节码与类的加载篇/image-20220427154451847.png)

## 3-javap的用法

![image-20220427161323382](JVM中篇-字节码与类的加载篇/image-20220427161323382.png)

![image-20220427162624576](JVM中篇-字节码与类的加载篇/image-20220427162624576.png)

![image-20220427162646398](JVM中篇-字节码与类的加载篇/image-20220427162646398.png)
...
![image-20220427163131009](JVM中篇-字节码与类的加载篇/image-20220427163131009.png)

3、jclasslib展示的内容：

## 5-总结

![image-20220427163148406](JVM中篇-字节码与类的加载篇/image-20220427163148406.png)
...
![image-20210508233536076](JVM中篇-字节码与类的加载篇/5c2a8d904287373990cffe9b82428daa.png)
...
![image-20210508234020104](JVM中篇-字节码与类的加载篇/a17ef03e0783c664a51491aafde85d2a.png)
...
![image-20210509001319088](JVM中篇-字节码与类的加载篇/8266c05b4b1506d4c456b427b90b1b75.png)
...
![image-20210509002525647](JVM中篇-字节码与类的加载篇/f3485b5ca6cb750454230270021fc68a.png)
