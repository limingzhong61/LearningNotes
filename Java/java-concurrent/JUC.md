...
简洁版：
**持锁线程**执行**ReentrantLock.signal方法**，**唤醒**在等待队列（ConditionObject抽象的等待队列）中的**第一个可以被唤醒的节点，也就是等待状态waitStatus的值为CONDITION  -2 的 Node节点**，**修改该Node节点等待状态waitStatus为0**，然后将该节点**插入到AQS的抽象的阻塞队列的尾部**，并修改**该节点的前驱节点的等待状态waitStatus 改为唤醒状态SIGNAL -1**，**等待状态waitStatus 为唤醒状态SIGNAL -1的节点表示其可以唤醒它的后继节点线程。**
...
执行 transferForSignal 流程，将**将该Node节点 的 waitStatus 改为 0，然后将该 Node 加入 AQS 队列尾部，**然后将**唤醒线程的前驱节点的等待状态waitStatus 改为唤醒状态SIGNAL -1**，等待状态为waitStatus 为 -1的节点表示可以唤醒下一个线程
...
* 执行 transferForSignal，**先将节点的 waitStatus 改为 0，然后加入 AQS 阻塞队列尾部**，将 插入节点的前驱节点(原阻塞队列的尾节点Thread-3 )的 waitStatus 改为 -1
  ...
  final boolean transferForSignal(Node node) {
      ...
      // 如果前驱节点被取消或者前驱节点不能设置状态为 Node.SIGNAL，就 unpark 取消当前节点线程的阻塞状态, 
...
CountDownLatch（倒计时锁，countDown 倒计时，latch门栓，可以翻译为锁）：**计数器**，用来进行线程同步协作，**等待所有线程完成。**

其中构造参数用来初始化等待计数值，await() 用来等待计数归零，countDown() 用来让计数减一

构造器：

* `public CountDownLatch(int count)`：初始化等待计数值,也就是唤醒需要的 down 几步
...
- 线程调用 countDownLatch.await() 等待其他线程完成任务：支持打断
-  尝试获取锁，判断当前state值是不是0，如果是则计算完成，直接退出；
  -  尝试获取锁，tryAcquireShared(1)
- doAcquireSharedInterruptibly(): 
  - 将调用latch.await()方法的线程 **包装成 SHARED 类型的 node 加入到 AQS 的阻塞队列**
  - **死循环：尝试获取锁，失败park住自己**
  - 如果当前节点的前驱节点是head头结点；再次尝试获取锁，判断当前state值是不是0，获取成功返回 1；如果获取锁成功，执行setHeadAndPropagate(node, r)方法
    - 将当前节点设置为新的 head 节点，前驱节点和持有线程置为 null
    - 当前节点是尾节点时 next 为 null，或者后继节点是 SHARED 共享模式；唤醒所有的等待共享锁的节点 doReleaseShared()->unparkSuccessor(h);
  - **获取锁成功（即判断state值是否为0）**，设置当前节点为 head 节点，并且向后传播（让其他阻塞的线程也能退出阻塞）
  - 如果不能获取锁，就将前驱节点设置为-1（-1能唤醒后继节点），并**再次循环一轮park住当前线程**
...
**构造函数**初始化链表 `last = head = new Node<E>(null);` 
