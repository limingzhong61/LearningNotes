...
> 常见的分布式任务的方案有：Quartz Cluster，XXL-Job，Elastic-Job等。: 综合**代码质量，License, 维护方，拓展性**等，选择的建议：
>
> 1. 如果**仅是小团队内部自用，可以采用XXL-Job**
> 2. 如果是**稍大一点的团队，建议使用ElasticJob或者基于ElasticJob进行二次开发**
...
> XXL-JOB是一个分布式任务调度平台，其核心设计目标是开发迅速、学习简单、轻量级、易扩展。现已开放源代码并接入多家公司线上产品线，开箱即用。实现案例[SpringBoot集成定时任务 - 分布式xxl-job方式]()
