...
![image-20210501102535142](4再谈类的加载器/fb51cabb2218d857a809a59918c5beec.png)
...
> **双亲委派模型优点：**安全  避免重复加载 保护程序 防止核心api被串改
...
从概念上来讲，自定义类加载器一般指的是程序中由开发人员自定义的一类类加载器，但是Java虚拟机规范却没有这么定义，而是将**所有派生于抽象类ClassLoader的类加载器都划分为自定义类加载器(Java语言实现)**。无论类加载器的类型如何划分，在程序中我们最常见的类加载器结构主要是如下情况：

![image-20210501164413665](4再谈类的加载器/0c43fb4a7da20038c8f56b42a1ddf802.png)
...
> 组合：特殊的关联，属于整体和部分的关系，是比聚合关系更强的关系，要求普通的聚合关系中代表整体的对象负责代表个体的对象的生命周期。

**注意：**

启动类加载器通过C/C++语言编写，而**自定义类加载器都是由Java语言编写的**，虽然扩展类加载器和应用程序类加载器是被jdk开发人员使用java语言来编写的，但是也是由java语言编写的，所以也被称为自定义类加载器
...
- 加载扩展类和应用程序类加载器，并指定为他们的父类加载器。
  ...
  ![image-20210501170011811](4再谈类的加载器/43431a1abff0e5f2bd7bfd20fe91e5f7.png)
  ![image-20210501170038212](4再谈类的加载器/5cd73234cd993408846ec7b4c2cb7238.png)
...
**执行结果：**
![image-20210501170425889](4再谈类的加载器/c72286acf05c7f86d1ea24f74e0c2a1e.png)
...
- 从java.ext.dirs系统属性所指定的目录中加载类库，或从JDK的安装目录的jre/lib/ext子目录下加载类库。如果用户创建的JAR放在此目录下，也会自动由扩展类加载器加载。
  ...
  ![在这里插入图片描述](4再谈类的加载器/ba54af96e744eb99a9248d13e55a7e3c.png)
...
**执行结果：**
  ...
  ![img](4再谈类的加载器/b21313ee65acb4f01f1d03b88529e9f5.png)
...
![image-20210501171206453](4再谈类的加载器/19dd9595afeb6543593ef14161fc1bb8.png)
...
![image-20210501173110637](4再谈类的加载器/00148a792adcf05932114dff408f82b5.png)
...
![image-20220504153042487](4再谈类的加载器/image-20220504153042487.png)![image-20220504153047542](4再谈类的加载器/image-20220504153047542.png)
...
![image-20210501174730756](4再谈类的加载器/53f634bada23b3400a303bfcc6e11a31.png)
...
![img](4再谈类的加载器/a77b6bca10308e6b9be35b1b2dfc84bc.png)
...
如果一个类加载器在接到加载类的请求时，它首先不会自己尝试去加载这个类，而是把这个**请求任务委托给父类加载器去完成**，依次递归，如果**父类加载器可以完成类加载任务，就成功返回**。只有**父类加载器无法完成此加载任务时，才自己去加载。**

**本质**

规定了类加载的顺序是：**引导类加载器先加载，若加载不到，由扩展类加载器加载**，若还加载不到，才会由系统类加载器或自定义的类加载器进行加载。

> 父**类加载器一般只有一个 而子类加载器可能有多个**, 从下往上好一点

![image-20210501175529542](4再谈类的加载器/41cbfd9ba1da676a7f05dae0dac3abae.png)

![img](4再谈类的加载器/c1526b63e95852b48551b4f136ebbd6f.png)
...
![img](4再谈类的加载器/814235dcce5471c2a527e82bafcf21c7.png)
...
IBM公司主导的JSR-291(即OSGiR4.2)实现模块化热部署的关键是它自定义的类加载器机制的实现，每一个程序模块(osGi中称为Bundle)都有一个自己的类加载器，当需要更换一个Bundle时，就把Bund1e连同类加载器一起换掉以实现代码的热替换。在oSGi环境下，类加载器不再双亲委派模型推荐的树状结构，而是进一步发展为更加复杂的**网状结构**。
...
小结：这里，我们使用了“被破坏”这个词来形容上述不符合双亲委派模型原则的行为，但**这里“被破坏”并不一定是带有贬义的。只要有明确的目的和充分的理由，突破旧有原则无疑是一种创新。**
...
![image-20210501182003439](4再谈类的加载器/9ae876265c85af4e431932647993dab7.png)
...
![image-20210501182608205](4再谈类的加载器/71caf905a0ca13866e24419b7faa14ee.png)
...
![image-20210501182626963](4再谈类的加载器/b93f0829e8340131a49738f8843307e4.png)

## 6.3. JDK1.2时期

在Java1.2版本中，再次改进了安全机制，增加了**代码签名**。不论本地代码或是远程代码，都会按照用户的安全策略设定，由类加载器加载到虚拟机中权限不同的运行空间，来实现差异化的代码执行权限控制。如下图所示JDK1.2安全模型：

![image-20210501182652378](4再谈类的加载器/81919ed70977f2b57ef5a11481c68abc.png)
...
![image-20210501182740197](4再谈类的加载器/cf22059daec45f370ae3afb33b870c1f.png)
...
1. 扩展机制被移除，扩展类加载器由于向后兼容性的原因被保留，不过**被重命名为平台类加载器(platform class loader)。**可以通过classLoader的新方法getPlatformClassLoader()来获取。
...
![img](4再谈类的加载器/323cfcda53f98034ed15372c0ea43685.png)
...
![img](4再谈类的加载器/cb23791a5fb1bf1a4c8a28d6a3179e84.png)

![img](4再谈类的加载器/ef9b83abcdb9f54d0f0ec7d15f0adc44.png)

![img](4再谈类的加载器/192fda50804d35e7d1b44dc61a65ede1.png)

![img](4再谈类的加载器/f07a455ec275a6503bfad070ae3d9ffb.png)
